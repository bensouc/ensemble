...
RSpec.describe WorkPlanSkillsController, type: :controller do
  ...
  describe "#create" do
    ...
    it "creates a new WorkPlanSkill with challengeKind attached to a WorkplanDomain and WorkPlan" do
      create(:work_plan_domain, work_plan: work_plan)
      skill = create(:skill, level: work_plan.work_plan_domains.first.level, domain: work_plan.work_plan_domains.first.domain)
      valid_params = {
        skill: skill,
        kind: "exercice",
        status: "new",
      ...
      }.to change(work_plan.work_plan_domains.first.work_plan_skills, :count).by(1)
    end
    ...
    it "redirects to the work_plan Show after adding a new WorkPlanSkill" do
      ...
      post :create, params: valid_params
      expect(response).to redirect_to(work_plan_path(work_plan) + "#challenge_" + work_plan.work_plan_domains.first.work_plan_skills.last.challenge.id.to_s)
    end
  end

  describe "#eval_update" do
    let(:work_plan_domain) { create(:work_plan_domain, work_plan: work_plan) }
    let(:skill) { create(:skill, level: work_plan_domain.level, domain: work_plan_domain.domain) }
    let(:work_plan_skill) {create(:work_plan_skill, skill: skill, kind: "ceinture", status: "new",work_plan_domain:)}
